...
bool hiddentriple(Cell **p_cells, int value1, int value2, int value3) {
    int count = 0;
...
bool checkcell(Cell *p_cells, int value1, int value2, int value3) {
    ...
    for (int i = 0; i < num_value - 2; i++) {
        for (int j = i + 1; j < num_value - 1; j++) {
            for (int k = j + 1; k < num_value; k++) {
                if (hiddentriple(p_cells, hidden_triple_value_array[i], hidden_triple_value_array[j], hidden_triple_value_array[k])) {
                    for (int l = 0; l < BOARD_SIZE - 2; l++) {
                        if (checkcell(p_cells[l], hidden_triple_value_array[i], hidden_triple_value_array[j], hidden_triple_value_array[k]) &&
                            checkcell(p_cells[l + 1], hidden_triple_value_array[i], hidden_triple_value_array[j], hidden_triple_value_array[k]) &&
                            checkcell(p_cells[l + 2], hidden_triple_value_array[i], hidden_triple_value_array[j], hidden_triple_value_array[k])) 
                            {
