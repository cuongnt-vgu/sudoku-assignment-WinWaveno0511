...
bool hiddenpair(Cell **p_cells, int value1, int value2){
    int count = 0;
...
bool cell(Cell *p_cells, int value1, int value2){
...
void find_hidden_pair(Cell **p_cells, HiddenPair *p_hidden_pair, int *p_counter)
{
    ...
    for (int i = 0; i < num_value - 1; i++)
    {
        ...
        {
            if (hiddenpair(p_cells, hidden_pair_value_array[i], hidden_pair_value_array[j]))
            {
                for (int k = 0; k < BOARD_SIZE; k++)
                {
                    if (cell(p_cells[k], hidden_pair_value_array[i], hidden_pair_value_array[j]))
                    {
